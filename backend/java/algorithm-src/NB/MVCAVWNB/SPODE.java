...
public class SPODE extends AbstractClassifier {
	...
	public int[] distributionForInstance_idx(Instance instance, int numBins) {
		...
		int[] aa = new int[(m_NumAttributes - 1) * m_NumClasses];

		// double T = 2;

		for (int i = 0; i < m_NumAttributes - 1; i++) {
			int[] idx = Utils.sort(result[i]);// ascend
			int[] rank = new int[m_NumClasses];
			// // arr 9,2,3,8,4,2
			// // idx 1 5 2 4 3 0
			// // rank 0 1
			// for (int j = idx.length - 1; j >= 0; j--) {
			// int cur_id = idx[j];
			// double cur_val = result[i][cur_id];

			// int k = j;
			// while (k >= 0 && Utils.eq(result[i][k] , cur_val)) {
			// k--;
			// }
			// // jump repeated
			// for (int kk = k + 1; kk <= j; kk++) {
			// rank[idx[kk]] = idx.length - j;
			// }
			// j = k + 1;
			// }
			...
			for (int j = 0; j < m_NumClasses; j++) {
				rank[idx[j]] = m_NumClasses - j - 1;// [0,numClasses-1]
			}
			for (int j = 0; j < m_NumClasses; j++) {
				int id = (int) (1.0 * (rank[j] - 1) / numBins) + 1;
				if (rank[j] == 0)
					id = 0;
				// aa[i * m_NumClasses + j] = rank[j];
				aa[i * m_NumClasses + j] = id;
			}
		}
	...
	public void distributionForInstance_c_p_rank(Instance instance, String[] pred_label, double[][] pred_probs,
			double[][] probs_rank) {
		...
		for (int i = 0; i < m_NumAttributes - 1; i++) {
			int[] idx = Utils.sort(result[i]);// ascend
			int[] newRank = new int[m_NumClasses];

			int cnt = 0;
			newRank[idx[0]] = cnt;
			for (int j = 1; j < m_NumClasses; j++) {
				if (result[i][idx[j - 1]] != result[i][idx[j]])
					cnt++;
				newRank[idx[j]] = cnt;
			}
			// for (int j = 0; j < m_NumClasses; j++)
			// System.out.print(newRank[j] + " ");
			// System.out.println();

			for (int j = 0; j < m_NumClasses; j++) {
				 probs_rank[i][j] = (newRank[j]*1.0/(cnt+1));
//				probs_rank[i][j] = newRank[j];
			}
	 ...
	 * 得到类标签和类概率的排序值
	 */
	public void distributionForInstance_c_rank(Instance instance, String[] pred_label, double[][] pred_rank) {
...
//			 System.out.println();
			...
			for (int j = 0; j < m_NumClasses; j++) {
				 pred_rank[i][j] = (newRank[j]*1.0/(cnt+1));
//				pred_rank[i][j] = newRank[j];
			}
	...
	public void distributionForInstance_c_p_x(Instance instance, String[] pred_label, double[][] group_probs,
			int numGroups) {
		int numAttrs = m_NumAttributes - 1;
		if (numGroups <= 0) {
			throw new IllegalArgumentException("numGroups must be > 0");
		}
		double prob;
		int numRoot;
		double[][] attrProbs = new double[numAttrs][m_NumClasses];
		int[] attIndex = new int[m_NumAttributes];
		for (int att = 0; att < m_NumAttributes; att++) {
			if (att == m_ClassIndex) {
				attIndex[att] = -1;
		...
		for (int classVal = 0; classVal < m_NumClasses; classVal++) {
			...
			for (int parent = 0; parent < m_NumAttributes; parent++) {
				...
				attrProbs[parent][classVal] = prob;
				classProbSum += prob;
			}
			if (numRoot < 1) {
				double fallback = (m_ClassCounts[classVal] + 1.0) / (m_NumInstances + m_NumClasses);
				for (int son = 0; son < m_NumAttributes; son++) {
					if (attIndex[son] == -1) {
						continue;
					}
					fallback *= (m_ClassAttAttCounts[classVal][attIndex[son]][attIndex[son]] + 1.0)
							/ (m_ClassCounts[classVal] + m_NumAttValues[son]);
				}
				for (int parent = 0; parent < numAttrs; parent++) {
					attrProbs[parent][classVal] = fallback;
				}
			}
		}
		// Step 1: first output numAttr class labels.
		for (int a = 0; a < numAttrs; a++) {
			Utils.normalize(attrProbs[a]);
			if (pred_label != null && a < pred_label.length) {
				pred_label[a] = String.valueOf(Utils.maxIndex(attrProbs[a]));
			}
		}
		// Step 2: then merge probabilities into numGroups.
		for (int a = 0; a < numAttrs; a++) {
			int groupId = a % numGroups;
			for (int c = 0; c < m_NumClasses; c++) {
				group_probs[groupId][c] += attrProbs[a][c];
			}
		}
	}

	public void diversity_JSD(Instance instance, double[][] differences) {
		double prob;
		int numRoot;
		double[] probs = new double[m_NumClasses];
		double[][] result = new double[m_NumAttributes - 1][m_NumClasses];
		...
		for (int i = 0; i < m_NumAttributes - 1; i++) {
			for (int j = i + 1; j < m_NumAttributes - 1; j++) {
				for (int c = 0; c < m_NumClasses; c++) {
					double p1 = result[i][c];
					double p2 = result[j][c];
					double m = (p1 + p2) / 2;
					differences[i][j] += (p1 * Math.log(p1 / m) + p2 * Math.log(p2 / m)) / 2;
