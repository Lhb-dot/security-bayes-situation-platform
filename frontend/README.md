...
| 模块 | 功能 | 需求条款 | 说明 |
|:-----|:-----|:---------|:-----|
| 用户与权限 | 用户登录 / 角色鉴权 | 6.2 / 6.5 | 全局路由守卫 + 角色落地页（管理员/用户→绑定场景看板）；mock 层强制校验，前端隐藏仅体验 |
...
| 态势展示 | 角色化首页 | 7.0 | 管理员平台总览/场景数据画像 + 场景用户我的工作台 |
...
| `/` | — | 按角色重定向（系统管理员→`/overview`，其余账号→绑定场景看板） | — |
| `/overview` | 平台总览 | 系统管理员平台运行总览 | 仅 SUPER_ADMIN |
